...
Note: designed for 'int' and 'unw' files... but wouldn't take much effort to
handle others...
...
def format_coord(x, y):
    ...
    """
    X = np.flipud(plt.gcf().get_axes()[0].get_images()[
                  0].get_array())  # messy but works!
...
def format_coord2(x, y):
    ...
    """
    Amp = np.flipud(plt.gcf().get_axes()[0].get_images()[
                    0].get_array())
...
def load_data(igramPath, args):
    ...
    if igramPath.endswith('int'):
        # sometimes rsc LENGTH is off by 1,,, so load just based on width
        amp, phs = load_cpx(igramPath, dims[1])
    else:
...
def create_double_browser(igramsDict, args):
    ...
    # Add cursor
    if args.cursor:
        multi = MultiCursor(fig.canvas, (ax1, ax2), color='k',
    ...
    def onpress(event):
        if event.key not in ('n', 'p', 'right', 'left'):
            return
        if event.key == 'n' or event.key == 'right':
            newval = sIG.val + 1
        elif event.key == 'p' or event.key == 'left':
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='browse ROI_PAC images in a directory')
    # Positional arguments
    parser.add_argument(
        'files', help='file string with wildcards (e.g. "int*/filt*32*int")')

    # Optional arguments
    parser.add_argument('-m', '--cmap', default='bwr',
        help='matplotlib colormap string')

    parser.add_argument('-c', '--clim', type=float, default=(None, None),
        nargs=2, metavar=('cmin', 'cmax'), help='manual limits for colorbar')

    parser.add_argument('-x', '--cursor', action='store_true', default=False,
        help='show cursorin both amp and phs windows (need to also use -a)')

    parser.add_argument('-a', '--amp', action='store_true',
        default=False, help='show radar amplitude alongside phase')

    parser.add_argument('-d', '--displacement', action='store_true',
        default=False, help='convert unwrapped phase to displacement [m]')

    parser.add_argument("-v", "--verbose", default=False,
        help="print interferogram to terminal", action="store_true")
    ...
    print('\n*Use right and left arrow keys to change cycle images. '
    ' Or click on the slider.\n ')

    if args.displacement is True and not args.files.endswith('unw'):
        print('Warning: only .unw (unwrapped) files can be converted '
        'to displacement... [m]\n')
